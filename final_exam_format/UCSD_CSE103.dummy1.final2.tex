...
%   {\large \bf { Section:  } }
\par
\noindent{\large \bf {Assignment final2  due 12/17/2013 at 11:45am PST}}
\par\noindent \bigskip
...
{\pgmlSetup
{\pgmlIndent\let\pgmlItem=\pgmlbulletItem
\pgmlItem{}$P(Y=3|X=1) = $ \mbox{\parbox[t]{5ex}{\hrulefill}}
...
{\pgmlSetup
{\pgmlIndent\let\pgmlItem=\pgmlbulletItem
\pgmlItem{}$P(Y=2|X=3) = $ \mbox{\parbox[t]{5ex}{\hrulefill}}
\pgmlRule{100\pgmlPercent}{1\pgmlPixels}%
...
\pgmlItem{}Write an exact expression for the probability that the number of heads is equal to 0.4$\times$100=40.  \mbox{\parbox[t]{9.5ex}{\hrulefill}}
\pgmlItem{}What Z-score corresponds to getting 40 heads? \mbox{\parbox[t]{8ex}{\hrulefill}}
\pgmlItem{}What is the central limit theorem approximation for the probability that the number of heads is 40 or higher assuming that the coin is fair? \mbox{\parbox[t]{8ex}{\hrulefill}}

\par}%

\pgmlItem{}Suppose that instead of n=100 times we flip the coin n=10000 times.
{\pgmlIndent\let\pgmlItem=\pgmlcircleItem
\pgmlItem{}What Z-score corresponds to getting 0.4$\times$10000=4000 heads? \mbox{\parbox[t]{8ex}{\hrulefill}}
\pgmlItem{}What is the central limit theorem approximation for the probability that the number of heads is 4000 or higher assuming that the coin is fair?\mbox{\parbox[t]{8ex}{\hrulefill}}
\par}%
...
{\pgmlSetup
{\pgmlIndent\let\pgmlItem=\pgmlalphaItem
\pgmlItem{}As we increase the number of coin flips $n$, does the probability of the event {\bfseries{}``the number of heads is larger than 0.4$\times n$''} increase,decrease or stay the same? \mbox{\parbox[t]{7ex}{\hrulefill}}
\par}%
\par}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% WeBWorK Online Homework Delivery System
...
\vskip\baselineskip
{\pgmlIndent\let\pgmlItem=\pgmlcircleItem
\pgmlItem{}What is the probability that it will not receive any request within the first 3 seconds of some particular minute? \mbox{\parbox[t]{8.5ex}{\hrulefill}}
\vskip\baselineskip
\pgmlItem{}What is the probability that it will receive exactly 3 requests during the same period of time? \mbox{\parbox[t]{11ex}{\hrulefill}}
\vskip\baselineskip
\pgmlItem{}What is the probability that the time between the first and the second requests received in a minute would be smaller than 5 seconds?  \mbox{\parbox[t]{7ex}{\hrulefill}}
...
\vskip\baselineskip
Using Chebyshev's inequality, give an upper bound on $P(|Y| > 30)$?  \mbox{\parbox[t]{14ex}{\hrulefill}}
\par}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% WeBWorK Online Homework Delivery System
% Copyright © 2000-2007 The WeBWorK Project, http://openwebwork.sf.net/
...
\par}%
\leavevmode\\\relax 
\includegraphics[width=0.8\linewidth]{/opt/webwork/courses/UCSD_CSE103/templates/setFinals/mixture_cdf_1.png}
...
\par}%
\vskip\baselineskip
Let's say we have algorithm that determines whether any integer is prime.  For an integer input that takes up $n$ bits, the algorithm takes $n$ seconds to run with probability $1/2$.  With probability $1/2$ the algorithm takes 1 second to run.  What is $T(n)$, in seconds?  \mbox{\parbox[t]{3.5ex}{\hrulefill}}.
\vskip\baselineskip
...
Fixing some $n$, apply Markov's inequality to get an upper bound on $P(X_n >= cT(n))$: \mbox{\parbox[t]{3ex}{\hrulefill}}.\pgmlBreak
What is $c$ such that $P(X_n >= cT(n)) <= 0.05$?  \mbox{\parbox[t]{3.5ex}{\hrulefill}}
\vskip\baselineskip
Thus an algorithm A' is as follows:
{\pgmlIndent\let\pgmlItem=\pgmlbulletItem
\pgmlItem{}Run A until time \mbox{\parbox[t]{3.5ex}{\hrulefill}}T(n).  If A has completed, return the correct value.
